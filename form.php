...
// form.php - secure contact form handler with anti-spam and security measures
...
$FROM = 'noreply@dejvcodes.com'; // must be a valid address on your domain
...
// Validate CSRF token from form-token.php
...
// Occasionally clean up old rate limit files (older than 7 days)
...
$maxReq = 5;  // max 5 requests per minute per IP
...
// Keep only recent requests inside the time window
$hits = array_values(array_filter(
  $hits,
  fn($t) => is_int($t) && $t > ($now - $window)
));

// Block if too many requests were made recently
...
if ($subject === '' || mb_strlen($subject) > 200) {
  fail(400, 'Fill in the subject.');
}

if ($message === '' || mb_strlen($message) > 4000) {
  fail(400, 'Message is empty / too long.');
}

// --- sanitize values for email headers (prevent header injection) ---
...
$body =
  ...
  "Email: {$emailSafe}\n" .
  "Subject: {$subjectSafe}\n" .
  "Privacy Accepted: {$privacy} - client accepted the privacy policy\n" .
  "---\n" .
  $message . "\n";
...
// Use return-path via -f (recommended for WEDOS mail delivery)
$ok = mail(
  $TO,
  $subjectEnc,
  $body,
...
// Invalidate the CSRF token after successful submission
unset($_SESSION['csrf']);
